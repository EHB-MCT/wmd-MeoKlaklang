...
export default function DailyEntryForm() {
  ...
  ========================= */
	useEffect(() => {
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [food, water, sleepHours, walks, playtimeMinutes, aloneHours, poop, behavior, emotion, appetite, energyLevel, vomit, meds, stressSignals, painSignals, trainingDone, leftAloneTooLong, ownerConcern]);
  ...
  ========================= */
	const handleSubmit = async (e) => {
		...
		const fields = [food, water, sleepHours, walks, playtimeMinutes, aloneHours, poop, behavior, emotion, appetite, energyLevel, ownerConcern];
						...
						{dogs.length === 0 ? (
							...
							<div className="dog-cards">
								{dogs.map((dog) => {
									...
									return (
										<div key={dog._id} className={`dog-card ${isSelected ? "selected" : ""}`} onClick={() => selectDog(dog._id, "sidebar_card")}>
					...
					{dogSelected && (
						<div className="recent-entries-section">
							...
							<div className="stats-grid">
								...
								<div className="stat-card">
									<div className="stat-number">{entries.filter((e) => e.dogId === dogId && e.date === new Date().toISOString().split("T")[0]).length}</div>
								...
								<div className="mini-calendar">
									...
									<div className="calendar-grid">
										{getDaysInMonth(currentMonth).map((day, index) => {
											const isSelected = day === parseInt(selectedDate.split("-")[2], 10) && currentMonth.getMonth() === new Date(selectedDate).getMonth() && currentMonth.getFullYear() === new Date(selectedDate).getFullYear();
							...
							<div className="form-header-info">
								<div className="form-title">
									...
									<div className="selected-date-display">
										...
										{selectedDate === new Date().toISOString().split("T")[0] && <span className="today-badge">Vandaag</span>}
										{isPastDateFromToday(selectedDate) && selectedDate !== new Date().toISOString().split("T")[0] && <span className="past-date-badge">Verleden</span>}
									</div>
								</div>
